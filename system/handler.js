...
module.exports = async (m, sock, store) => {
  if (m.key.jid === "status@broadcast") {
    ...
    console.log(
      chalk.green.bold("– View status:* " + m.pushName),
    );
    return;
  }
        ...
        await plugin(m, {
          ...
          .then(async (a) => {
            if (plugin?.settings?.limit && !isPrems && !m.isOwner) {
              ...
              m.reply(
                `> *WARNING:* Este comando está limitado\n> *- Tu limite actual:* ${db.list().user[m.sender].limit} limites\n> *- INFO:* Tus limites se reinician a las 02:00 am todos los dias.`,
              );
            }
          });
      }
...
//          if (!jid[0].exists) continue;
          let caption = "*– Error Detectado*\n"
          caption += `> *comando:* ${m.command}\n`
          caption += `> *Ubicación:* ${name}`
        ...
        m.reply("*– *Error Detectado* *\n !*\n> Lo siento un error de código no me permite ejecutar el comando\n> ¡He enviado un informe del error a mi propietario, agradecería la espera!");
      } else {
