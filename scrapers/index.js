...
class Scraper {
  ...
  load = async () => {
    ...
    for (let i of data) {
      ...
      try {
        ...
        console.log(chalk.magenta.bold("- Error al cargar el Scraper: " + e));
  ...
  watch = async () => {
    ...
    watcher.on('add', async (filename) => {
      ...
      this.#src[name] = require(filename);
      console.log(chalk.magenta.bold("- Nuevo Scraper agregado: " + name));
      return this.load();
    });
    ...
    watcher.on('change', (filename) => {
      ...
      console.log(chalk.magenta.bold('- Scraper modificado: ' + name));
      return this.load();
    });
    ...
    watcher.on("unlink", (filename) => {
      ...
      let name = filename.split('/').pop().replace('.js', '');
      delete this.#src[name];
      console.log(chalk.magenta.bold('- Scraper eliminado: ' + name));
      return this.load();
    });
  }
