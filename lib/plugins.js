  ...
  load = async () => {
    ...
    for (const filename of files) {
      ...
      try {
        ...
        console.log(`– Error detectado [ ${relativePath} ] : ` + e.message);
  ...
  watch = async () => {
    ...
    watcher
      .on("add", async(filename) => {
        ...
        console.log("[ New ] Plugin Recién detectado : " + filename);
        this.plugins[relativePath] = require(filename);
        return this.load();
      })
      ...
      const relativePath = path.relative(process.cwd(), filename);
       if (require.cache[filename]) {
        ...
        console.log("[ Change ] Cambios de código en Archivos : " + filename);
        delete require.cache[filename];
        this.plugins[relativePath] = require(filename);
        return this.load();
      })
      .on("unlink", (filename) => {
        const relativePath = path.relative(process.cwd(), filename);
        console.log("[ Delete ] Eliminar correctamente : " + filename);
        delete this.plugins[relativePath];
      });
   };
  }
